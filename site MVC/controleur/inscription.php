...
if (isset($_POST["mailU"]) && isset($_POST["mdpU"]) && isset($_POST["pseudoU"])) {
    ...
    if ($_POST["mailU"] != "" && $_POST["mdpU"] != "" && $_POST["pseudoU"] != "" && $inscrit==false) {
        $mailU = $_POST["mailU"];
        $pseudoU = $_POST["pseudoU"];
        $testMail = getUtilisateurByMailU($mailU);
        $testPseudo = getUtilisateurByPseudoU($pseudoU);
        if($testMail || $testPseudo){
            $msg="Mail ou pseudo déjà utilisé";
        }
        else{
        $mailU = $_POST["mailU"];
        $mdpU = $_POST["mdpU"];
